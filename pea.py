...
nadya = LINETCR.LINE()
#nadya.login(qr=True)
nadya.login(token='ErjIijQQKZfwH7FwkaBc.KBtylGF7T07g2h8DyG87Ja.RcYggTKYcuCm2tmAksugaiUjY17TRNMBdA4RU1KxmUk=')
nadya.loginResult()
...
#                            Name = summon(op.param2)
        ...
        if op.type == 26:
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention2"] == True:          
                    contact = nadya.getContact(msg.from_)
                    cName = contact.displayName
                    balas = ["Ada perlu apa kisanak ngetag kiyai vicky!","Kenapa kisanak tag saya kiyai Fbl vicky","assalamualaikum " + cName + "ada  apa,kisanak tag Kiyai Vicky!"]
                    ret_ = random.choice(balas)
                    name = re.findall(r'@(\w+)', msg.text)
                    mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                    mentionees = mention['MENTIONEES']
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention3"] == True:          
                    contact = nadya.getContact(msg.from_)
                    cName = contact.displayName
                    balas = ["Assalamualaikum " + cName + ", ada apa kisanak Ngetag saya kiyai vicky!"]
                    balas1 = "Salam kenal kisanak. . ."
                    ret_ = random.choice(balas)
                    image = "http://dl.profile.line-cdn.net/" + contact.pictureStatus
                    name = re.findall(r'@(\w+)', msg.text)
                    mention = ast.literal_eval(msg.contentMetadata['MENTION'])
		...
		if wait["Simi"] == True: md+="╠➩✔️ Simisimi : On\n"
            ...
            elif "Kuchiyose no jutsu" == msg.text.lower():
